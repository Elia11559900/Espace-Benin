...
function editerLocataire(id) {
    ...
    database.ref(`entreprises/${currentUser.uid}/locataires/${id}`).once('value', (snapshot) => {
        ...
        if (locataire) {
            document.getElementById("nom").value = locataire.nom; // Ajout du nom
...
function calculerStatistiquesLogements() {
 ...
 database.ref(`entreprises/${currentUser.uid}/logements`).once('value', (snapshot) => {
   ...
   snapshot.forEach((childSnapshot) => {
            const logement = childSnapshot.val();
            // Assurez-vous que logement.date existe et est une date valide
            const logementDate = logement.date ? new Date(logement.date) : null;
            const logementMois = logementDate ? logementDate.getMonth() + 1 : null;
            const logementAnnee = logementDate ? logementDate.getFullYear() : null;
...
// Gestion des modales (Termes et Confidentialité pour admin)
document.getElementById('show-admin-terms').addEventListener('click', function(event) {
    event.preventDefault();
    document.getElementById('admin-terms-modal').style.display = 'block';
});

document.querySelector('#admin-terms-modal .close-button').addEventListener('click', function() {
    document.getElementById('admin-terms-modal').style.display = 'none';
});

document.getElementById('show-admin-privacy').addEventListener('click', function(event) {
    event.preventDefault();
    document.getElementById('admin-privacy-modal').style.display = 'block';
});

document.querySelector('#admin-privacy-modal .close-button').addEventListener('click', function() {
    document.getElementById('admin-privacy-modal').style.display = 'none';
});

// Fermeture modale en cliquant en dehors (admin)
